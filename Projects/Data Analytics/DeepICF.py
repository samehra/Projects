...
def load_rating_file_as_list(filename):
    ...
    with open(filename, "r") as f:
        ...
        while line != None and line != "":
            arr = line.split("\t")
            user, item, rating, timestamp = int(arr[0]), int(arr[1]), int(arr[2]), int(arr[3])
            ratingList.append([user, item, rating, timestamp])
            line = f.readline()
    return ratingList
...
ratings = load_rating_file_as_list('Data/ml-1m.train.rating')
ratings = pd.DataFrame(ratings)

ratings1 = load_rating_file_as_list('Data/pinterest-20.train.rating')
ratings1 = pd.DataFrame(ratings1)

pct=20
df1 = (ratings.groupby(ratings[0], group_keys=False).apply(lambda x: x.nlargest(int(len(x) * pct/100), 3)))
np.savetxt(r'Data\ml-300k.txt', df1.values, fmt='%d')

df2 = (ratings1.groupby(ratings1[0], group_keys=False).apply(lambda x: x.nlargest(int(len(x) * pct/100), 3)))
np.savetxt(r'Data\pinterest-300k.txt', df2.values, fmt='%d')
