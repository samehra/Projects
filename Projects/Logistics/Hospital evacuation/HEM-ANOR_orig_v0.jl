...
m = Model(solver =CplexSolver(CPX_PARAM_TILIM=1000))
...
for  i in 1:nRH, p in 1:nPT, v in 1:nVT, t in 1:nT
    evacRisk[i,p,v,t] = t*threatRisk[p] + travelRisk[v,p]*(recHos[i,1]+2)
end

println(nPT ,", ",nT,", ",nVT)

@variable(m, x[1:nRH,1:nPT,1:nVT,1:nT] >= 0)
@variable(m, y[1:nRH,1:nVT,1:nT],  Int >= 0)
